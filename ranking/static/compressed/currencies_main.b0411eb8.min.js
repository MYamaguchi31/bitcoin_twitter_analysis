...
var assetTableOptions={"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"string","aTargets":[1],"render":constrainName},{"sType":"string","aTargets":[2]},{"sType":"currency","aTargets":[3]},{"sType":"price","aTargets":[4]},{"sType":"volume","aTargets":[5]},{"sType":"totalsupply","aTargets":[6]},{"sType":"percent","aTargets":[7]},{"bSortable":false,"aTargets":[8]},{"asSorting":["desc","asc"],"aTargets":[3,4,5,6,7]},],"language":getLanguageOptions()};initResponsiveTable('#assets',assetTableOptions,
[0,1,4,7,3,6,5,8,2]);$('#markets-table').dataTable({"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"innerstring","aTargets":[1]},{"sType":"innerstring","aTargets":[2]},{"sType":"volume","aTargets":[3]},{"sType":"price","aTargets":[4]},{"sType":"percent","aTargets":[5]},{"sType":"string","aTargets":[6]},{"asSorting":["desc","asc"],"aTargets":[3,4,5]},],"language":getLanguageOptions()});var currenciesVolume=$('#currencies-volume').DataTable({"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"string","aTargets":[1]},{"sType":"string","aTargets":[2]},{"sType":"volume","aTargets":[3]},{"sType":"volume","aTargets":[4]},{"sType":"volume","aTargets":[5]},{"asSorting":["desc","asc"],"aTargets":[3,4,5]},],"order":[],"language":getLanguageOptions()}); currenciesVolume.on('order.dt search.dt',function(){currenciesVolume.column(0,{order:'applied'}).nodes().each(function(cell,i){cell.innerHTML=i+1;});}).draw();$('#trending-recently-added').DataTable({"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"order":[],"aoColumnDefs":[{"sType":"innerstring","aTargets":[0]},{"sType":"string","aTargets":[1]},{"sType":"percent","aTargets":[2]},{"sType":"currency","aTargets":[3]},{"sType":"price","aTargets":[4]},{"sType":"totalsupply","aTargets":[5]},{"sType":"volume","aTargets":[6]},{"sType":"percent","aTargets":[7]},{"asSorting":["desc","asc"],"aTargets":[3,4,5,6,7]},],"language":getLanguageOptions()});$('.trending-gainers-table, .trending-losers-table').DataTable({"bFilter":false,"bInfo":false,"bLengthChange ":false,"bPaginate":false,"bStateSave":false,"order":[],"aoColumnDefs":[{"bSortable":false,"aTargets":[0]},{"sType":"innerstring","aTargets":[1]},{"sType":"string","aTargets":[2]},{"sType":"volume","aTargets":[3]},{"sType":"price","aTargets":[4]},{"sType":"percent","aTargets":[5]},{"asSorting":["desc","asc"],"aTargets":[3,4,5]},],"language":getLanguageOptions()});jQuery.extend(jQuery.fn.dataTableExt.oSort,{"totalsupply-pre":function(a){if($(a).data("supply")){a=$(a).data("supply");}
a=parseFloat(a)
if(isNaN(a)){a=-1;}
...
return a;},"percent-asc":function(a,b){return a-b;},"percent-desc":function(a,b){return b-a;}});jQuery.extend(jQuery.fn.dataTableExt.oSort,{"innerstring-pre":function(a){a=$(a).text();return a;},"innerstring-asc":function(a,b){return a-b;},"innerstring-desc":function(a,b){return b-a;}});});function toggle_currency(currency){var currency_lowercase=currency; if(currency_lowercase=="btc"){$.each([$('.market-cap'),$('.price'),$('.volume')],function(){selector_type=this.selector
...
$.each([$('.percent-1h'),$('.percent-24h'),$('.percent-7d')],function(){$.each(this,function(key,value){slug=$(this).closest('tr').attr("id");if(slug=="id-ethereum"&&currency_lowercase=="eth"){var amount=Formatter.toLocaleString(0.00,{minDecimalPlaces:2,maxDecimalPlaces:2});$(this).html(amount+"%");}else{convert_percent($(this),data_symbol)}});});}
function toggle_native(){$.each([$('.price'),$('.volume')],function(){selector_type=this.selector
$.each(this,function(key,value){amount=$(this).data("native");if(amount==null){amount="N/A";}
else if(amount!="?"){amount=parseFloat(amount)
if(selector_type==".price"){amount=format_fiat(amount);}else{amount=format_market_cap(amount);}
if(selector_type!=='.price'){amount=abbreviateIfMobile(amount);}}
$(this).html(amount);});});}
function toggle_platform(){$.each([$('.market-cap'),$('.price'),$('.volume')],function(){selector_type=this.selector
...
function abbreviateNumber(number,decPlaces){ decPlaces=Math.pow(10,decPlaces);var abbrev=[polyglot.t("K"),polyglot.t("M"),polyglot.t("B"),polyglot.t("T")];for(var i=abbrev.length-1;i>=0;i--){var size=Math.pow(10,(i+1)*3);if(size<=number){number=Math.round(number*decPlaces/size)/decPlaces;if((number===1000)&&(i<abbrev.length-1)){number=1;i++;}
number=Formatter.toLocaleString(Number(number),{minDecimalPlaces:2,maxDecimalPlaces:2});number+=" "+abbrev[i];break;}}
return number;}
...
function renderCurrency(options){var currency=options.currency;var label=options.label||$('[data-currency="'+currency+'"]').html()||currency.toUpperCase();if(currency=='native'){updateCurrencySwitch(label,'native');toggle_native();}else if(currency=='platform'){updateCurrencySwitch(label,'platform');toggle_platform();}else{updateCurrencySwitch(label,currency);toggle_currency(currency);}
toggleSupply();adjustColumnWidthForChangedText();}
function rerenderCurrency(){var currency=$('#currency-switch-button').data('currency');renderCurrency({currency:currency});}
$(document).ready(function(){var currencyValue=Currency.get();updateFiatOptions(currencyValue);renderCurrency({currency:currencyValue.toLowerCase(),label:currencyValue});$(".price-toggle").click(function(){var currency=$(this).data('currency');var label=$(this).html();renderCurrency({currency:currency,label:label});dataLayer.push({'event':'customEvent','eventCategory':'Local Currency Dropdown','eventAction':'Currency Selected','eventLabel':currency});});});Currency.onChange(function(e,currency){updateFiatOptions(currency);renderCurrency({currency:currency.toLowerCase(),label:currency});});